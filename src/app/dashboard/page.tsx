        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <HolographicCard className="lg:col-span-2 flex flex-col md:flex-row items-center justify-between gap-6 load-hidden delay-200">
            <div className="space-y-4">
              ...
              <p className="text-muted-foreground max-w-md">
                Welcome back, Voyager. Your learning matrix is synchronized. Engage with AI-driven tasks or enter the Game Zone to expand your neural pathways.
              </p>
          ...
          <HolographicCard className="flex flex-col items-center justify-center text-center load-hidden delay-300">
             <div 
              ...
              style={{ background: 'radial-gradient(circle, hsl(var(--secondary)/0.3) 0%, transparent 70%), conic-gradient(hsl(var(--primary)) 270deg, hsl(var(--muted)) 0deg)' }}
            >
              <div className="absolute inset-2 rounded-full bg-background flex items-center justify-center">
                 <Trophy className="w-12 h-12 text-primary/70 text-pulse" />
              </div>
            </div>
            <p className="text-muted-foreground mt-4 text-sm">Weekly XP</p>
            <p className="text-3xl font-bold font-headline text-pulse">1,250</p>
          </HolographicCard>
        </div>

        {/* Stats Row */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <StatCard icon={CheckSquare} label="Tasks Done" value="42" delay="delay-300" />
          <StatCard icon={Clock} label="Study Time" value="12h 45m" delay="delay-400" />
          <StatCard icon={Percent} label="Accuracy" value="92.5%" delay="delay-500" />
        </div>
      </div>
    </div>
  );
